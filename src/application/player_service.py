...
class PlayerService:
    ...
    def player_init(self, id: int, username: str):
        ...
        if not player:
            ...
            newPlayerResources = PlayerResources(player_id=id)
            newPlayerGun = PlayerGun(player_id=id, gun_id=1, is_equipped=True)
            newPlayerHull = PlayerHull(player_id=id, hull_id=1, is_equipped=True)
            ...
            for i in range(2, 8):
                anotherPlayerGun = PlayerGun(player_id=id, gun_id=i, is_equipped=False)
                self.db.add(anotherPlayerGun)

            # Добавляем все оставшиеся корпуса (неэкипированные)
            for i in range(2, 6):
                anotherPlayerHull = PlayerHull(player_id=id, hull_id=i, is_equipped=False)
