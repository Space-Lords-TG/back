...
Противники подбираются так, чтобы общая мощь кораблей отличалась друг от друга не более, чем на {config["game"]["match_power_threshold"]}. 
Урон пушек во время боя случайным образом меняет значение в интервале ±{config["game"]["damage_deviation"]}%. 
Награда за бой зависит от результата и от разницы мощи кораблей. 
Полученный здоровьем урон не восстанавливается автоматически — необходимо отремонтировать корпус.\n\n
"""
...
Скорость: {speed:.0f}
Урон: {damage:.0f}
Шанс крита: {crit_rate:.2f}%
Крит. урон: {crit_damage:.2f}x
Защита: {armor:.0f}
Щиты: {shields:.0f}
Манёвренность: {maneuver:.2f}
...
Вместимость:
{current_crystal_capacity} (К), {current_metal_capacity} (М) -> {next_crystal_capacity} (К), {next_metal_capacity} (М)
